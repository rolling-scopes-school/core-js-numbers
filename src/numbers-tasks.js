...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const dotProductUV = x1 * x2 + y1 * y2;
  const magnitudeU = Math.sqrt(x1 ** 2 + y1 ** 2);
  const magnitudeV = Math.sqrt(x2 ** 2 + y2 ** 2);

  return Math.acos(dotProductUV / (magnitudeU * magnitudeV));
}
...
function getFibonacciNumber(index) {
  ...
  for (let i = 2; i <= index; i += 1) {
    currentFi = minusOneFi + minusTwoFi;
    minusOneFi = minusTwoFi;
    minusTwoFi = currentFi;
  }
  return currentFi;
}
...
function getSumOfDigits(num) {
  let sum = 0;
  Array.prototype.forEach.call(num.toString(), (n) => {
    sum += +n;
  });
  return sum;
}
...
function getSumOfNumbers(x1, x2, x3) {
  return Math.round((x1 + x2 + x3) * 100000000) / 100000000;
}
