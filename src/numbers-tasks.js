...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const compressedCos = Math.max(-1, Math.min(1, cosine));
  const angle = Math.acos(compressedCos);
  return angle;
}
...
function roundToPowerOfTen(num, pow) {
  ...
  const divideByExpo = num / exponent;
  const roundResult = Math.round(divideByExpo);
  const multResult = roundResult * exponent;
  return multResult;
}
...
function getSumOfDigits(num) {
  ...
  return numStr.map(Number).reduce((a, b) => a + b, 0);
}
