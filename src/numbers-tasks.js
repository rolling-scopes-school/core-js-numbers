...
function roundToPowerOfTen(num, pow) {
  let res = num;
  if (+String(num % 10 ** pow).charAt(0) >= 5) {
    res = num + 1 * 10 ** pow;
  }
  return res - (num % 10 ** pow);
}
...
function getSumOfDigits(num) {
  let sum = 0;
  const intArr = Array.from(String(num));
  // eslint-disable-next-line no-restricted-syntax, guard-for-in, prefer-const, func-names
  intArr.forEach(function (entry) {
    sum += +entry;
  });
  return sum;
}
...
function toExponential(number, fractionDigits) {
  ...
  //   first = String(number)[0] + String(number)[1];
  // } else {
  //   // eslint-disable-next-line no-const-assign, prefer-destructuring
  //   first = String(number)[0];
  // }
  // let second = String(number).substring(
  //   first.length,
  //   first.length + fractionDigits
  // );
  // const length = String(number).substring(first.length);
  // if (String(number)[first.length + fractionDigits] >= 5) {
  //   second = +second + 1;
  // }
  // return `${first}.${second}e+${length.length}`;
